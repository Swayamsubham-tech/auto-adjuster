...
CLASSIFIER_CHECKPOINT = os.environ.get("CLASSIFIER_CHECKPOINT", "checkpoints/damage_classifier_best.pth")
...
async def infer(
    ...
    claim_id: str = Form(None, description="Optional claim ID; auto-generated if omitted"),
):
    """Accepts a video upload, runs the full CV pipeline, returns the Damage JSON."""
    if not os.path.exists(SAM_CHECKPOINT):
        raise HTTPException(status_code=503, detail=f"SAM checkpoint not found at {SAM_CHECKPOINT}. Service not ready.")
    if not os.path.exists(CLASSIFIER_CHECKPOINT):
        raise HTTPException(status_code=503, detail=f"Classifier checkpoint not found at {CLASSIFIER_CHECKPOINT}. Run Week 2's training first.")
    ...
    try:
        # We need to explicitly add src to the path for the API to find our modules
        import sys
        if "src" not in sys.path:
            sys.path.insert(0, "src")
            
        result = run_full_cv_pipeline(
            video_path=tmp_video_path, sam_checkpoint=SAM_CHECKPOINT, sam_model_type=SAM_MODEL_TYPE,
            classifier_checkpoint=CLASSIFIER_CHECKPOINT, claim_id=claim_id,
        )
