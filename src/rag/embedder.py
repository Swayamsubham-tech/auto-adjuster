    ...
    if not os.path.exists(filepath):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(
                ...
                "covered under comprehensive insurance, subject to a $500 deductible. Replacement of the "
                "bumper is covered if structural integrity is compromised.\n\n"
                "SECTION 2: LIGHTING EXCLUSIONS\nShattered headlight lenses are covered ONLY IF caused "
                "by an active collision with another vehicle. Headlight damage from road debris or weather "
                "is explicitly excluded from this policy."
            )
...
def get_cloud_embedding(text):
    ...
    # Uses HuggingFace's free public inference API to avoid local AI execution
    ...
    try:
        ...
        # If the free API is rate-limited, fallback to a mathematical dummy vector so FAISS testing can proceed
        return [0.05] * 384
