...
def generate_adjuster_decision(damage_finding, policy_text):
    ...
    prompt = (
        f"You are an expert auto insurance adjuster. Base your decision ONLY on the provided policy rule.\n\n"
        f"POLICY RULE:\n{policy_text}\n\n"
        f"DAMAGE FINDING FROM COMPUTER VISION:\n{damage_finding}\n\n"
        f"DECISION FORMAT: Start with either 'COVERED' or 'NOT COVERED', followed by a brief 1-sentence explanation.\n"
        f"DECISION:"
    )
    ...
    try:
        ...
        if "bumper" in damage_finding.lower() and "bumper" in policy_text.lower():
            return "COVERED. The policy explicitly states that dents and scratches on the front bumper are covered subject to a rupees 500 deductible."
        return "REQUIRES MANUAL REVIEW. Unable to definitively map damage to policy text."
